...
listfiles=dir(paste(DirRawTexts,sep=''))
...
for (i in 1:length(stext)){
        file.copy(paste(DirRawTexts,'/','plaintxt-',stext[i],'.txt',sep=''),
                  paste(DirRawTexts,'/','article-',stext[i],'.txt',sep='')
                  ,overwrite=T)
}
rm(i)
...
if (!length(mtext)==0){
        for (i in 1:length(idsm)){
                ...
                for (page in 1:nidsm){
                        ...
                        article[page]=readLines(paste(DirRawTexts,'/','plaintxt-',idsm[i],'-',page,'.txt',sep=''),encoding='UTF-8')
                }
                ...
                #                         }
                write.csv(article,
                          paste(DirRawTexts,'/','article-',idsm[i],'.txt',sep=''),
                          ,fileEncoding='UTF-8'
                          ,quote=F                                
                )
        }
