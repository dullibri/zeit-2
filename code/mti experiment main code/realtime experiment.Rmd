...
for (horizon in c(14,15)){# horizon=1
        ...
        for (vint.num in 1:length(sets)){# vint.num=1
                ...
                elanet[[vint.num]]=elanet.ind(set,y,yx,horizon,alpha=1)
                                ...
                                forecast['ar',extract.ar]=unlist(
                                        olsbic3(y,yx,horizon,max.lag,ic=ic)[extract]
                                        )
                                p1=forecast['ar','p1']       
                                ...
                                for (i in 1:ncol(set)){# i=1
                                        ...
                                        resu=unlist(
                                                olsbic1setgetting2(y,xx,horizon,max.lag,p1=p1,ic=ic)[extract]
                                                )
                                        if (length(resu)>0){
        ...
        if (max.obs<0){
                write.csv(lassm,paste(DirCode,'/results/rec_elanet_ranking',ic,target,'_h',horizon,'.csv',sep=''))
                
                save(forecast.all,file=paste(DirCode,'/results/rec_',ic,target,'_h',horizon,'.RData',sep=''))
                }
        if (max.obs>0){
                write.csv(lassm,paste(DirCode,'/results/elanet_ranking',ic,target,'_h',horizon,'.csv',sep=''))
                
                save(forecast.all,file=paste(DirCode,'/results/',ic,target,'_h',horizon,'.RData',sep=''))
                }
