...
source(paste(auxcodedir,'/mtoq.R',sep=''))
...
target='cons' # core inflation
ic='aic' 
disregard=''# a variable that is a nearly perfect substitute for target.
max.lag=4 # maximum lag length to be considered 
max.obs=25 # maximum number of past observations to be considered (rolling estimation);
...
# estimation. 
start.2000=1 # data used starting in 2001:1 as media data are avaiable from here
retest.on=TRUE
...
# drop a lot of zeit indices that are not communicable
drop=unlist(read.csv(paste(DirCode,'/results/evaluation_leave_out.csv',sep='')
                     ,stringsAsFactors=F
                     ,header=F))
drop=drop[-grep('^mean$|^median$',drop)]
...
# dropping sets for which no monthly realtime data are available
...
for (horizon in 1:4){# horizon=1
        ...
        #   elanet=list()
        for (vint.num in 1:length(setsq)){# vint.num=1
                setq=setsq[[vint.num]]
                month.set.num=names(setsq)[vint.num]
                
                set.rt=sets[[month.set.num]]
                set.last.date=names(sets[month.set.num])
                ...
                set=mtoq(set)
                overlap=row.names(setq)%in%row.names(set)
                overlap.quarters=row.names(setq)[overlap]
                set[,colnames(setq)]=NA
                set[overlap.quarters,colnames(setq)]=setq[overlap,]
                ...
                yx=400*log(y.raw/lag.exact(y.raw,1))
                ...
                if (start.2000==1){
                        start=grep('2000-1',row.names(set))
                ...
                #       set=set[,estnames]
                #     }
                dropcols=which(colnames(set)%in%drop)
                set=set[,-dropcols]
