...
for (h in 1:max.hor){# h=14 
        ...
        for (i in 1:nrep){#i=1
                New=matrix(tsboot(lossm, function(x) x, R = 1, l = 40, sim = "geom")$t,nrow=nrow(lossm))
...
#                 Tspastar[i]=max(c(max(Tstats.new[,1]),0))
        ...
        Hansen.pv=sum(Tspastar>Tspa,na.rm=T)/sum(is.na(Tspastar>Tspa)==F)
        WT.pv=sum(WT>Vl)/nrep
        
        Hstat=function(x){y=f_Hansen_2005_Test_statistic(x,blockparam)}
        Tspa=max(c(max(T),0))
        
        boot=tsboot(lossm, function(x) x, R = 1, l = 20, sim = "geom")
        boot=tsboot(lossm, Hstat, R = 1, l = 20, sim = "geom")
        
        
        New=matrix(t(boot$t),ncol=ncol(lossm))
        T1=apply(New,2,f_Hansen_2005_Test_statistic,blockparam)
        Loss = as.matrix(lossm[,1])
        
        t=f_Hansen_2005_Test_statistic(Loss,blockparam)
        Out_MSE = f_White_Reality_Check(alternative, 1, benchmark, NRep, 0, Block_Param)
        #         Out_MAE = f_White_Reality_Check(alternative, 3, benchmark, NRep, 0, Block_Param)
        white.p[[h]]=PValue = Out_MSE[[1]]
