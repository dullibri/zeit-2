...
source(paste(auxcodedir,'/olsbic2setgetting3.R',sep=''))
...
for (horizon in 1:13){# horizon=2
#         forecast.all=vector('list',length(sets))
        load(paste(DirCode,'/Results/IP_h',horizon,'.RData',sep=''))
#         elanet=list()
        for (vint.num in 1:length(sets)){# vint.num=1
#               getting the best model
                fca=forecast.all[[vint.num]]
                bestmodel=fca['IFO.EXP',]
...
#                 elanet[[vint.num]]=elanet.ind(set,y,yx,horizon,alpha=1)
                                ...
                                colssubset=grep('MT.',colnames(set))
                                nsubset=length(colssubset)
                                subset=set[,colssubset]
                                forecast.plus=data.frame(matrix(NA,nrow=34,length(extract)))
                                colnames(forecast.plus)=extract
                                row.names(forecast.plus)=c(colnames(subset))
                                
                                       
                                
                                for (i in 1:nsubset){# i=1
                                        bestres=set[,row.names(bestmodel)]
                                        xx=cbind(yx,bestres,subset[,i,drop=F])
                                        p1=bestmodel[1,'p1']
                                        p2=bestmodel[1,'p2']
                                        resu=unlist(
                                                olsbic2setgetting3(y
                                                                   ,df=xx
                                                                   ,horizon
                                                                   ,max.lag
                                                                   ,p1=p1
