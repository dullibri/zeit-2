...
for (jj in 1990:2015){#jj={#:2015 jj=1990
        ...
        for (k in 1:length(liste_jahr)){#k=3
                ...
                Ergebnis=data.frame(matrix(NA,1,14))
                ...
                for (i in 1:Narticle_issue){# i=1
                        ...
                        issue=gsub(paste(DirRawTexts),'',sFolderTexte)
                        issue=gsub('/','',issue)
                        year=strsplit(issue,'\\.')[[1]][1]
                        issue=strsplit(issue,'\\.')[[1]][2]
                        Ergebnis[i,13:14]=c(issue,year)
