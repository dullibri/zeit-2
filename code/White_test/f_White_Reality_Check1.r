...
library(tseries)
library(boot)
ts.alternative=ts(alternative,c(1900,1),freq=12)
number.of.bootstraps=1
blocklength=12
t=lapply(1:170,function(x) tsbootstrap(ts.alternative[,x],nb=number.of.bootstraps,m=blocklength))
ts.benchmark=ts(benchmark,c(1900,1),freq=12)
tf=function(x){x-benchmark}
t=tsboot(ts.alternative[,1],tf,R=1000)
t1=t$t
t=alternative[,1]
boo
