...
for (jj in 1990:2015){#jj=1990 jj=2014 jj=2005
        ...
        for (k in 1:length(liste_jahr)){#k=24
                ...
                Ergebnis=data.frame(matrix(NA,length(art_num),5))
                colnames(Ergebnis)=c('title','number','date','keywords','link')
                
                Ergebnis[,'number']=paste(liste_jahr[k],art_num,sep='.')
                for (i in 1:Narticle_issue){# i=7
                        ...
                        keywords_index=regexec(paste('url content=(http://www.zeit.de/[online/]{7}?[0-9]{4}/[0-9]{2}/[^<]*)\\>?'),text)
                        Ergebnis[i,'link']=regmatches(text,keywords_index)[[1]][2]
                        
                        if (is.na(Ergebnis[i,'link'])==T){
                                keywords_index=regexec(paste('url content=(http://www.zeit.de/[0-9]{4}/[0-9]{2}/[^<]*)\\>?'),text)
                                Ergebnis[i,'link']=regmatches(text,keywords_index)[[1]][2]  
                        }
                        
                }
                yi=gsub('\\.','_',liste_jahr[k])
                write.csv(Ergebnis,paste(DirCode,'/data/zeit indikatoren/meta_',yi,'.csv',sep=''),row.names=F)
        }
...
write.csv(ER,paste(DirCode,'/data/zeit indikatoren/meta_all.csv',sep=''),row.names=F)
