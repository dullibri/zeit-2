olsbic2setgetting3 <-function(y,df,horizon,max.lag,ic='bic',p1,p2){
        ...
        x3=as.matrix(x3[xy.complete,])   
              
        y=y[xy.complete,]
        # getting optimal model with x2
        olsaux2=function(k){
                res=ols(cbind(y,x1[,1:p1,drop=F],x2[,1:p2,drop=F],x3[,1:k,drop=F]))
                return(res)
        }
        sel=data.frame(1:max.lag)
        res=t(mapply(olsaux2,sel[,1]))
        partial=sapply(res,length)>2
        # if only part of the lags are feasable res will be a list with varying lengths, thus:
        if (sum(partial)>0&nrow(res)!=max.lag){
                res=res[partial,drop=F]
                res=t(sapply(res,function(x) x))
        }
        p3=which.min(res[,ic])
        if (res[p3,ic]==10^10){
                ret=vector(mode='list',length=length(extract))
                names(ret)=extract
                ret[extract]=-999                
                return(ret)}
        result=res[p3,]
