olsbic1setgetting2 <-function(y,df,horizon,max.lag,ic='bic',p1){
        ...
        lagvec=which(partial) # which ones were calculated?
        # which of these is the minimum
        if (sum(partial)>0&nrow(res)!=max.lag){
                res=res[partial,drop=F]
                res=t(sapply(res,function(x) x))
        }
        # translating minimum back to complete list of lags
        p2=lagvec[which.min(res[,ic])]
        # setting -999 if no model can be computed
        if (res[p2,ic]==10^10){
