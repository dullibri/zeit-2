olsbic3 <-function(y,df,horizon,max.lag,ic='bic'){
        # building on ols, lag.exact, lag.hormax and plugin.values, the functions estimates a 
        # ols model for predicting # of horizons ahead with a maximum lag of max.lag for up to
        # 3 regressors x1, x2, and x3. The optimal lag lengths p1, p2, and p3 are estimated based on 
        # bic (alternative: 'aic'). (all possible models are estimated)
        ...
        if (nvar==3){
                x3=lag.hormax(df[,3,drop=F],horizon,max.lag)      
        }
        
        # restricting to complete cases
        if (nvar==1){xy=cbind(y,x1)}
        if (nvar==2){xy=cbind(y,x1,x2)}
        if (nvar==3){xy=cbind(y,x1,x2,x3)}
        xy.complete=complete.cases(xy)
        nobs=sum(xy.complete)
        x1=as.matrix(x1[xy.complete,])
        if (nvar>=2){x2=as.matrix(x2[xy.complete,])}
        if (nvar==3){x3=as.matrix(x3[xy.complete,])}     
        y=y[xy.complete,]
        
        # getting optimal model using bic
        res=list()
        if (nvar==1){# creating function
                olsaux=function(i){
                        res=ols(cbind(y,x1[,1:i,drop=F]))
                        return(res)
                }
                sel=data.frame(1:max.lag)#creating selection vector
                res=t(mapply(olsaux,sel[,1]))
                
        }
        if (nvar==2){
                olsaux=function(i,j){
                        res=ols(cbind(y,x1[,1:i,drop=F],x2[,1:j,drop=F]))
                        return(res)
                } 
                sel=rep(1:max.lag,each=max.lag)
                sel=data.frame(cbind(sel,rep(1:max.lag,max.lag)))
                res=t(mapply(olsaux,sel[,1],sel[,2]))
        }
        
        if (nvar==3){
                olsaux=function(i,j,k){
                        res=ols(cbind(y,x1[,1:i,drop=F],x2[,1:j,drop=F],x3[,1:k,drop=F]))
                        return(res)
                }
                sel=rep(rep(1:max.lag,each=max.lag),max.lag)
                sel=data.frame(cbind(sel,rep(rep(1:max.lag,max.lag),max.lag)),rep(1:max.lag,each=max.lag^2))
                res=t(mapply(olsaux,sel[,1],sel[,2],sel[,3]))
        }        

        best=which.min(res[,ic])
        result=res[best,]
        p1=sel[best,1]
        result$p1=p1
        pv1=plugin.values(df[,1,drop=F],max.lag)[1:p1]
        pv=pv1
        if (nvar>=2){p2=sel[best,2]
                     result$p2=p2
                     pv2=plugin.values(df[,2,drop=F],max.lag)[1:p2]
                     pv=cbind(pv,pv2)
        }
        if (nvar==3){p3=sel[best,3]
                     result$p3=p3
                     pv3=plugin.values(df[,3,drop=F],max.lag)[1:p3]
                     pv=cbind(pv,pv3)
        }
