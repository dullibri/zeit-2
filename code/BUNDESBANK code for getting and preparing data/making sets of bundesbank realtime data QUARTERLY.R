...
variable=data.frame(matrix(c('Q.DE.Y.A.AG1.CA010.C.A','gdp'
                      ,'Q.DE.Y.A.CA1.BA100.C.A','cons'
                      ,'Q.DE.Y.A.CD1.BAA00.A.I','inv'
                      ,'Q.DE.Y.A.CX1.CA010.C.A','exp'
                      ,'Q.DE.Y.A.CM1.CA010.C.A','imp'
                      )
                    ,ncol=2
                    ,byrow=T
                    )
...
dates=data.frame(year=rep(1995:2015,each=4))
dates$month=rep(1:4,length(1995:2015))
dates$number=NA
dates$vintage=NA
...
#         variable.name=var.used$variable[1]
...
var.used[var.used$label%in%c('ORD-C','ORD-I','CPI','CPI-EX','IP','IP-VORL','IP-CONST','TARIF'),'Dln']=1
...
var.used.s=var.used
c("name","id","L","D","Dln","D2ln",'lag',"code")%in%colnames(var.used)
